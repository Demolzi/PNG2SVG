...
class RegionPanel(QWidget):
    def __init__(
        ...
        self.table.setHorizontalHeaderLabels(["选", "名称", "类型", "显示", "状态"])
    ...
    def set_regions(
        ...
        try:
            ...
            for index, region in enumerate(regions, start=1):
                ...
                self.table.setItem(row, 1, _item(f"{index}. {region.display_name}", region.id))
                self.table.setItem(row, 2, _center_item(region.shape_label, region.id))
                self.table.setItem(row, 3, _center_item("是" if region.visible else "否", region.id))
                self.table.setItem(row, 4, _center_item("合法" if region.is_valid else "非法", region.id))
    ...
    def _build_export_settings(self) -> QGroupBox:
        ...
        export_all_button = QPushButton("导出全部 SVG")
